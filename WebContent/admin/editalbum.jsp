...
<html>
	<head>
		...
		<title>Lyrics Admin - Modifica ${albumRequested.author.name} - ${albumRequested.title}</title>
        <link href='http://fonts.googleapis.com/css?family=PT+Sans' rel='stylesheet' type='text/css'>
        <link href="${pageContext.request.contextPath}/css/bootstrap.min.css" rel="stylesheet" >
        <link href="${pageContext.request.contextPath}/css/interface.css" rel="stylesheet" >
        <link href="${pageContext.request.contextPath}/admin/css/editalbum.css" rel="stylesheet" >
	</head>
	<body>
		<!-- navbar top - header -->
		...
		<div class="container edit-body">
			<div class="row text-center title-row">
				<h1>Modifica: ${albumRequested.title}</h1>
			</div>
			<form action="<c:url value="/controller/AdminEditAlbum" />" method="post">
				<input type="hidden" name="id" value="${albumRequested.id}"/>
				<div class="row">
					<div class="col-md-2"></div>
					<div class="col-md-1"><label for="title">Titolo:</label></div>
					<div class="col-md-7"><input type="text" class="form-control" name="title" id="title" value="${albumRequested.title}"/><span>${titleErr}</span></div>
				</div>
				<div class="row">
					<div class="col-md-2"></div>
					<div class="col-md-1"><label for="year">Anno:</label></div>
					<div class="col-md-7"><input type="text" class="form-control" name="year" id="year" value="${albumRequested.year}"/></div>
				</div>
				<div class="row">
					<div class="col-md-2"></div>
					<div class="col-md-1"><label for="author">Autore:</label></div>
					<div class="col-md-7"><input type="text" class="form-control" name="author" id="author" value="${albumRequested.author.name}"/><span>${authorErr}</span></div>
				</div>
				<div class="row">
					<div class="col-md-2"></div>
