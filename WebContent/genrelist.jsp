...
<html>
	<head>
		...
		<title>Lyrics - Elenco Generi</title>
	...
	<body>
		...
		<div class="container result-body <c:if test="${fn:length(genres) < 9}">empty-body</c:if> personal-adaptor">
			<div class="row">
				<div class="col-md-12">
					<h1>Generi presenti</h1>
				</div>
			</div>
			<c:forEach var="genres" items="${genres}">
				<div class="row result">
					<div class="col-md-4"><a href="<c:url value="/controller/GetAllTracksByGenre?genreName=${genres.name}" />">${genres.name}</a></div>
				</div>
			</c:forEach>
		</div>
